...
const EmojiSelector = ({ sex, name, avatarUrl }) => {
  ...
  const UploadToOss = async (file) => {
    // 获取上传签名的 URL
    const response = await fetch("http://localhost:3000/oss-sign");
    // const { signedUrl } = await response.json();
    const signedUrl = "http://localhost:3000/upload";
    // 使用签名 URL 进行上传
    ...
    if (ossResponse.ok) {
      ...
      return { url: signedUrl.split("?")[0] }; // 获取文件的 URL
    } else {
  ...
  const handleFileChange = async (files) => {
    ...
    for (let file of files) {
      ...
      if (result) {
        uploadedFiles.push({ url: result.url });
      }
    }
  ...
  return (
    <div className="p-2 bg-[#fde5e9] shadow-md relative">
      ...
      <div className="bg-white rounded-md">
        ...
        <img src="https://wall-lh.oss-cn-shenzhen.aliyuncs.com/1.jpg" />
