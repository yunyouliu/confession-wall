...
const EmojiSelector = ({ sex, name, avatarUrl }) => {
  ...
  const handleClickOutside = (event) => {
    if (dropdownRef.current && !dropdownRef.current.contains(event.target)) {
      setIsDropdownOpen(false); // 点击在弹出层外部，关闭弹出层
    }
  };
    ...
    <div className="p-2 bg-[#fde5e9] shadow-md relative">
      ...
      {isDropdownOpen && (
        <div
          ...
          className="absolute bottom-0 left-1/2 transform -translate-x-1/2 mt-4 w-80 p-2 bg-white rounded shadow-lg z-50"
        >
      ...
      <div className="flex items-center mt-4 z-10">
        ...
        <div className="ml-auto flex items-center h-2">
          ...
          <Button color="success" className="rounded-3xl h-8 ml-2 px-4 z-10">
            评论
          </Button>
        </div>
      </div>
