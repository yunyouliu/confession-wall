...
const EmojiSelector = ({ sex, name, avatarUrl }) => {
  ...
  const UploadToOss = async (file) => {
    // 创建 FormData 对象来包含文件数据
    const formData = new FormData();
    formData.append("localFilePath", file); // 文件内容
    formData.append("objectName", file.name); // 文件名
  ...
  const handleFileChange = async (files) => {
    const uploadedFiles = [];
    setVisible(false); // 隐藏上传界面

    for (let file of files) {
      const result = await UploadToOss(file); // 调用上传函数
      if (result) {
        uploadedFiles.push({ url: result.url }); // 将上传后的文件 URL 保存到列表中
      }
    }

    setFileList(uploadedFiles); // 更新文件列表
  };
