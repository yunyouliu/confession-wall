...
const EmojiSelector = ({ sex, name, avatarUrl }) => {
  ...
  const UploadToOss = async (file) => {
    try {
      ...
      const ossResponse = await axios.post(url, formData);

      if (ossResponse.status === 200) {
        Toast.show({ content: "上传成功！", position: "bottom" });
        // console.log("上传成功：", ossResponse.data.data.url);
        setFileList([...fileList, { url: ossResponse.data.data.url }]);
        return { url: ossResponse.data.data.url }; // 获取文件的 URL
      } else {
      ...
      <div className="bg-white rounded-md">
        ...
        <div className={`justify-center items-center text-[#999999] p-2 `}>
          <ImageUploader
            ...
            ref={input}
            value={fileList}
            onChange={()=>{(files) => setFileList(files);setVisible(false)}}
            upload={UploadToOss}
          >
            <div
