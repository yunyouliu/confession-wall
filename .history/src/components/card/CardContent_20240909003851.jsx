...
const CardContent = ({ text, id }) => {
  ...
  const viewerRef = useRef(null); // 创建 ref
  ...
  return (
    <div className="text-left mt-2 py-2 indent-[20px] text-white text-base">
        ...
        {text.img.map((item, idx) => (
          <img
            ...
            onClick={() => {
              ...
              viewerRef.current?.open(); // 使用 ref 打开 ImageViewer
            }}
          />
        ))}
      </div>
      ...
      <ImageViewer.Multi
        images={text.img}
        ref={viewerRef} // 绑定 ref
        defaultIndex={index} // 使用当前选中的图片索引
        onClose={() => {
          viewerRef.current?.close(); // 关闭时调用 ref
        }}
      />
    </div>
  );
