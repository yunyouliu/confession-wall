...
const CardContent = ({ text, id }) => {
  ...
  return (
    <div className="text-left mt-2 py-2 indent-[20px] text-white text-base">
      ...
      {/* 如果内容溢出，显示展开/收起按钮 */}
      {isOverflowing && (
        <div className="text-white border-dashed text-center w-20 text-sm
         indent-0 ml-2  border-2 border-white mt-2 " onClick={handleToggleExpand}>
          {isExpanded ? "收起内容↑" : "展开内容↓"}
        </div>
      )}

     {text && text.img && text.img.length > 0 && <div className={`mt-2 p-2 ${getFlexClasses(text.img.length)}`}>
