  ...
  return (
    <>
      <div className="pl-2 pr-0.5  mt-3 justify-between">
        <Tabs
          defaultActiveKey="1"
          className=" bg-[#fde5e9] font-mono decoration-8 leading-3 overflow-hidden  rounded-md flex"
        >
          {tab.map((item) => {
            return (
              <Tabs.Tab
                // className="text-sm w-16"
                className={`text-sm w-16 ${item.title==="推广"?"ml-3":""}`}
                title={item.title}
                key={item.key}
              />
            );
          ...
          {tab.map((item) => {
            return (
              <Grid.Item key={item.key}>
                <div className=" bg-slate-200 h-[45px] m-0 leading-10 text-sm rounded-md  ">
